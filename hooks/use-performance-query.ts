	...
	> = {
		...
		queryFn:
			options.queryFn && typeof options.queryFn === 'function'
				? async (...args: Parameters<NonNullable<typeof options.queryFn>>) => {
						const label =
							performanceLabel || `Query: ${JSON.stringify(options.queryKey)}`
...
export const useQueryPerformance = () => {
	return {
		...
		getAverageFirstFetch: () => {
			...
			const firstFetchMetrics = metrics.filter(m =>
				m.name.startsWith('First Fetch:'),
			)
