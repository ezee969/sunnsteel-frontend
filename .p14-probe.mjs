...
p.on('console', m => {
	if (['error', 'warning'].includes(m.type()))
		msgs.push(
			`[${m.type()}] ${p.url().replace('http://localhost:3000', '')} :: ${m.text().slice(0, 180)}`,
		)
})
p.on('pageerror', e =>
	msgs.push(
		`[pageerror] ${p.url().replace('http://localhost:3000', '')} :: ${String(e).slice(0, 180)}`,
	),
)
...
const act = await p.evaluate(async () => {
	...
	const r = await fetch('http://localhost:4000/api/workouts/sessions/active', {
