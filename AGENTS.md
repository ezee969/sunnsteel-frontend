...
**It is effectively an SPA hosted on Next.js.** The Server Components are `app/page.tsx` (a redirect based on a cookie), `app/api/session/route.ts`, the six `loading.tsx` files, and the two static layouts around the public member-profile route. Every data-owning `page.tsx` remains `'use client'`; there is no RSC data fetching, no Server Actions, no `fetch` caching/`revalidate`, and no streaming. Data is fetched client-side via TanStack Query against the external backend. Keep this in mind before reaching for a Next.js server feature.

**`@sunsteel/contracts` is a published npm dependency (`^0.24.0` in [package.json](package.json)), not a `file:` link.** It must resolve from the registry because Vercel only clones this repo — pointing it at the local sibling (`file:../sunnsteel-contracts`) breaks the deploy (the shared types silently degrade to `any`). Trade-off: local edits to `../sunnsteel-contracts` are **not** picked up until you `npm publish` a new version and bump it here.
...
- `app/` — App Router. `(auth)` = public login/signup/OAuth callback; `(public)` = the signed-out-safe `/members/:identifier` profile surface; `(protected)` = the authed shell (dashboard, routines, workouts, profile, settings, search). `app/api/session/` is the only Route Handler.
