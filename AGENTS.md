...
- **Usernames are lowercase public identifiers, but profiles are not public routes yet.** `@sunsteel/contracts` owns the 3–30 character format and reserved-name list; the backend normalizes again and the database enforces format and uniqueness. Search returns name, username and avatar only — never email. `/users/:identifier` and `/profile/[[...userId]]` resolve readable usernames while retaining UUID compatibility, but the route remains authenticated until `PROF-12` ships; do not make Share Profile claim otherwise.
...
- **Profile privacy is enforced by the backend before sensitive reads.** Eight independent settings (`biography`, `location`, `trainingIdentity`, `workoutHistory`, `records`, `routines`, `achievements`, `bodyMetrics`) use `PUBLIC`/`FOLLOWERS`/`PRIVATE` and default to private. Owners always retain access; `FOLLOWERS` means the viewer follows the profile owner. The authenticated member-profile response includes a `viewerAccess` map and only queries/returns allowed biography, location, training identity, training summaries, personal records and body metrics; it never includes email. Biography, location and training identity have honest real, empty and private states. Routine and achievement visibility is persisted for `ROUT-04`/`PROF-07`/`PROF-08`, but no content is public yet. Keep the route authenticated until `PROF-12`.
