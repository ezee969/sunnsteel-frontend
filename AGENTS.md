...
Coverage stays in Node: **pure logic, auth event orchestration and API-contract tests**, 154 tests across twenty-three files, including `lib/api/types/workout-stats.test.ts` for local week boundaries, previous-performance, weight-unit conversion, training-location preferences, finite-inventory plate loading, live-record celebration, progression-change presentation, stale-session recovery, session-resolution copy, duration formatting and routine day-count pluralisation, and logout marker recovery in the auth suites. Network calls are mocked. [vitest.config.ts](vitest.config.ts) uses `environment: 'node'` on purpose — there is no jsdom and no React Testing Library, so **hooks and components cannot be rendered in tests** without first deciding to add them.
...
- **Profile privacy is enforced by the backend before sensitive reads.** Seven independent settings (`biography`, `location`, `workoutHistory`, `records`, `routines`, `achievements`, `bodyMetrics`) use `PUBLIC`/`FOLLOWERS`/`PRIVATE` and default to private. Owners always retain access; `FOLLOWERS` means the viewer follows the profile owner. The authenticated member-profile response includes a `viewerAccess` map and only queries/returns allowed biography, location, training summaries, personal records and body metrics; it never includes email. Biography and location have honest real, empty and private states. Routine and achievement visibility is persisted for `ROUT-04`/`PROF-07`/`PROF-08`, but no content is public yet. Keep the route authenticated until `PROF-12`.
