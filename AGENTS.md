...
Coverage stays in Node: **pure logic, auth event orchestration and API-contract tests**, 211 tests across thirty-six files, including `lib/api/types/workout-stats.test.ts` for local week boundaries, previous-performance, strength-trend ranges/chart mapping, exercise-performance, muscle-heatmap and volume-trend query serialization, heatmap matrix/intensity ranking, volume comparison summaries, weight-unit conversion, training-location preferences, finite-inventory plate loading, live-record celebration, progression-change presentation, stale-session recovery, session-resolution copy, profile sharing, relationship-list routes, copy and follow-state cache updates, dashboard primary-action precedence, empty-state actions, privacy-overview grouping, session-share links, field toggles and shared-recap sections, motion-preference resolution, the pre-paint motion script and splash timing, duration formatting, training-identity labels and routine day- and exercise-count pluralisation, and logout marker recovery in the auth suites. Network calls are mocked. [vitest.config.ts](vitest.config.ts) uses `environment: 'node'` on purpose — there is no jsdom and no React Testing Library, so **hooks and components cannot be rendered in tests** without first deciding to add them.
...
Route protection is in [middleware.ts](middleware.ts) and checks **only** `ss_session === '1'`. Protected prefixes: `/dashboard`, `/workouts`, `/routines`, `/profile`, `/progress`, `/settings`, `/search`. Unauthenticated hits redirect to `/login?redirectTo=<original>`; authenticated hits on `/login`/`/signup` redirect to `/dashboard`. [app/(protected)/layout.tsx](<app/(protected)/layout.tsx>) adds a second, client-side redirect: it bounces to `/login` when there is no Supabase session, or when the backend verification actually **failed** (`error && !user`) — not merely when the profile has yet to arrive.
...
- `app/` — App Router. `(auth)` = public login/signup/OAuth callback; `(public)` = the signed-out-safe `/members/:identifier` profile surface and `/shared/sessions/:token` workout shares; `(protected)` = the authed shell (dashboard, routines, workouts, profile, settings, search). `app/api/session/` is the only Route Handler.
...
- **Profile privacy is enforced by the backend before sensitive reads.** Eight independent settings (`biography`, `location`, `trainingIdentity`, `workoutHistory`, `records`, `routines`, `achievements`, `bodyMetrics`) use `PUBLIC`/`FOLLOWERS`/`PRIVATE` and default to private. Owners always retain access; `FOLLOWERS` means the viewer follows the profile owner. Authenticated and anonymous member-profile responses share the same `viewerAccess` shape and only query/return allowed biography, location, training identity, training summaries, personal records and body metrics; neither includes email. The anonymous endpoint treats every `FOLLOWERS` rule as denied and sends `Cache-Control: no-store`, so a privacy change cannot leave a shared page cached with stale access. Biography, location and training identity have honest real, empty and private states. Routine and achievement visibility is persisted for `ROUT-04`/`PROF-07`/`PROF-08`, but no content is public yet.
- **Training identity is profile metadata, not measurable programming.** `PROF-05` owns bounded broad goals, experience, disciplines, preferred routine style and up to five catalog-backed favorite exercises. `PROG-08` still owns measurable targets. Favorite exercises are stored by exercise ID and returned in user-defined order; never accept free-text favorites or treat labels as prescriptions.
- **`SHOW_QUICK_WORKOUT_ENTRY` is `false` on purpose and the code it guards is deliberately unreachable.** In [app/(protected)/workouts/page.tsx](<app/(protected)/workouts/page.tsx>), `handleStartEmptyWorkout` creates a routine whose only day has `exercises: []`, so the session it opens has nothing loggable and no way to add anything — that is why the button is hidden (`FIX-04`, decided 2026-09-07). It is **not** dead code awaiting cleanup: `LIVE-06` is `BLOCKED` on an undesigned replacement flow and resumes from this handler, so flipping the constant is the whole revert. Don't delete the flag, the handler, or the `isStartingEmpty` branch.
