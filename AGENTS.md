...
- **Workouts**: a local `qk` object inside [lib/api/hooks/useWorkoutSession.ts](lib/api/hooks/useWorkoutSession.ts) → `['workout','session','active']`, `['workout','session',id]`, `['workout','session',id,'previous-performance']`, `['workout','sessions',<serialized params>]`, and `['workout','progress','strength',exerciseId,from,to]`.
