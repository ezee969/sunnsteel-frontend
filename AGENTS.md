...
- **The strength chart is a best-set record frontier, not an every-session performance chart.** `/workouts/progress/strength` reconstructs only persisted `PERSONAL_RECORD` events for one exercise, carries the last pre-range event in as a baseline, and caps the response at 500 changes. `PROG-02` owns every session/set/RPE/note. Keep this read on the event stream and its partial `(userId, payload.exerciseId, occurredAt, id)` index; do not replace it with a lifetime `SetLog` scan.
