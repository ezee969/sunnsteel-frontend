...
- **Profile privacy is enforced by the backend before sensitive reads.** Five independent settings (`workoutHistory`, `records`, `routines`, `achievements`, `bodyMetrics`) use `PUBLIC`/`FOLLOWERS`/`PRIVATE` and default to private. Owners always retain access; `FOLLOWERS` means the viewer follows the profile owner. The authenticated member-profile response includes a `viewerAccess` map and only queries/returns allowed training summaries, personal records and body metrics; it never includes email. Routine and achievement visibility is persisted for `ROUT-04`/`PROF-07`/`PROF-08`, but no content is public yet. Keep the route authenticated until `PROF-12`.
