...
- **Progress has two intentionally different per-exercise reads.** `/workouts/progress/strength` reconstructs the capped `PERSONAL_RECORD` frontier and its pre-range baseline. `/workouts/progress/performance` paginates terminal sessions with completed work, then composes snapshot context, sets/RPE, session and prescription notes, and `PROGRESSION_CHANGED` events. Its exercise list also keeps non-weighted histories selectable. Keep both reads on their partial indexes; never turn either into an unbounded lifetime `SetLog` response.
