...
| ID     | Status     | Size | Work item                          | Expected behavior                                                                                                                                                                                                                                                                                                                                                                            | Repositories                                      |
| ------ | ---------- | ---- | ---------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------- |
| FIX-03 | `NEXT`     | M    | Correct kg/lb handling             | Canonical storage is already kilograms (`SetLog.weight`, `PersonalRecord.weight`, `WorkoutAnalyticsProjection.totalVolumeKg`), so this is presentation and input conversion, not a storage migration. Convert at the display and entry boundary, and never label a kilogram value as pounds. Revisit only if a future field stores a non-canonical unit. **Verified 2026-09-07: the defect is latent, and the fix has a required order.** `weightUnit` exists end to end -- the column, `users.service.ts` persisting it, and `weightUnit` on `UpdateProfileRequest` -- but no screen sets it, so every account keeps the `'KG'` default assigned at signup and a direct API `PATCH` is the only way to reach `'LB'`. Almost all of the UI hardcodes `kg`, which is honest under canonical storage; the one mislabel today is the Volume Lifted Total card in `app/(protected)/profile/[[...userId]]/page.tsx`, which labels an unconverted `totalVolumeKg` as `lbs` when the preference is `LB`. Consequence for sequencing: build the conversion boundary **before** exposing the unit selector that `PROF-01` notes Settings is missing. Exposing it first turns one latent mislabel into a live one across every render site at once. | FE; BE/CT only if a new preference field is added |
