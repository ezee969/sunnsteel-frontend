...
const weeklyGoalData = [
  // Block 1
  { week: 1, intensity: 0.70, sets: 5, reps: 5, amrapTarget: 10 },
  { week: 2, intensity: 0.75, sets: 5, reps: 4, amrapTarget: 8 },
  { week: 3, intensity: 0.80, sets: 5, reps: 3, amrapTarget: 6 },
  ...
  { week: 9, intensity: 0.80, sets: 5, reps: 3, amrapTarget: 6 },
  ...
  // Block 3
  { week: 15, intensity: 0.80, sets: 5, reps: 3, amrapTarget: 6 },
  { week: 16, intensity: 0.85, sets: 5, reps: 2, amrapTarget: 4 },
  { week: 17, intensity: 0.90, sets: 5, reps: 1, amrapTarget: 2 },
  { week: 18, intensity: 0.85, sets: 5, reps: 2, amrapTarget: 4 },
  { week: 19, intensity: 0.90, sets: 5, reps: 1, amrapTarget: 2 },
...
function calculateTMAdjustment(repsDone: number, targetReps: number, currentTM: number): number {
  ...
  for (let week = 1; week <= 21; week++) {
    ...
    if (week === 1) {
      ...
      if (prevWeekPerf && !prevGoalData.isDeload) {
        ...
        if (repsDone) {
          const newTM = calculateTMAdjustment(repsDone, prevGoalData.amrapTarget!, currentTM);
          const tmChange = newTM - currentTM;
          const tmChangePercent = (tmChange / currentTM) * 100;

          if (tmChange !== 0) {
            action = `Last week: ${repsDone}/${prevGoalData.amrapTarget!} reps. TM adjusted from ${currentTM.toFixed(1)} to ${newTM.toFixed(1)}kg (${tmChangePercent >= 0 ? '+' : ''}${tmChangePercent.toFixed(1)}%).`;
          } else {
            action = `Last week: ${repsDone}/${prevGoalData.amrapTarget!} reps. TM unchanged at ${currentTM.toFixed(1)}kg.`;
          }
          currentTM = newTM;
        }
